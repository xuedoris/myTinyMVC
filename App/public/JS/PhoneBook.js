...
function validate(){
	...
	if($('#phone_number').val() == '' || $('#phone_number').val().length < 7){
		$('#invalid_number').show();
		error = true
	}
	else
	...
	if(!error){
		$.post("http://localhost/myTinyMVC/index.php/PhoneBook/checkContact", $("#newContact").serialize(),
			function(data) {
				$("#exist").html(data);
			}
		)
